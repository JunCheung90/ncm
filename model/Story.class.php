<?php
	...
	class Story {
		...
		function __construct() {
			$this->userIdLower = 10001;
			$this->userIdUpper = 10200;
		...
		function us1New() {
			$user = new User();
			$userInfo = $user->getUserInfo($user->userIdLower+1001);
			$ownerInfo = $this->format_json($userInfo);
			$ownerInfo->contacts = array();
			
			$tempResult = $this->createUser();
			$contactArr = $tempResult->targetUsers;
			for($i=0; $i < count($contactArr); $i++) {
				$contactTemp = $this->format_json($contactArr[$i]);
				$ownerInfo->contacts[] = $contactTemp;
			}

			return $ownerInfo;
		}

		function writeIntoJsonFile() {
			for ($i = 1; $i < 2; $i++) {
				$str = Tool::json($this->us1New());
				$filePath = "us".$i.".json";
		...
		function format_json($sourceInfo) {
			$targetInfo = new stdClass;
			$targetInfo->name = $this->getName($sourceInfo);
			$targetInfo->phones = array();
			$phoneTemp = new stdClass;
			$phoneTemp->phoneNumber = $this->getMobilePhone($sourceInfo);
			$phoneTemp->isActive = Tool::randomPercentage() < 0.05 ? true : false;
			$targetInfo->phones[] = $phoneTemp;
			$targetInfo->emails = array();
			$targetInfo->emails[] = $this->getEmail($sourceInfo);
			$targetInfo->ims = array();
			$imTemp = new stdClass;
			$imTemp->type = "QQ";
			$imTemp->isActive = true;
			$imTemp->account = $this->getQQ($sourceInfo);
			$targetInfo->ims[] = $imTemp;

			$targetInfo->sn = array();
			$snTemp = new stdClass;
			$snArr = array("新浪微博", "腾讯微博", "豆瓣", "人人");
			$snTemp->type = Tool::randomInArray($snArr);
			$imTemp->accountName = null;
			$imTemp->accountId = Tool::randomInRange(10000000,1000000000);
			$imTemp->appKey = null;
			$targetInfo->sn[] = $snTemp;

			$targetInfo->addresses = array();
			$targetInfo->addresses[] = $this->getAddress($sourceInfo);
			$targetInfo->tags = array();
		...
		function createUser() {
			...
			for ($j = 0; $j < $this->userTotal; $j++) {
				...
				$userInfo->StructuredPostal = $originalUserInfo->StructuredPostal;
				...
				if($j < 9) {
					...
					$repeatUserInfo->StructuredPostal = $originalUserInfo->StructuredPostal;
		...
		function getAddress($contact) {
			return $contact->StructuredPostal[0]->FORMATTED_ADDRESS;
		}
